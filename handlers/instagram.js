const { getLocale, setState, clearState, validateUsername, formatNumberDisplay, isOwner } = require('../utils/helpers');
...
async function handleInstagramMenu(ctx) {
    try {
        ...
        const text = quote(locale.IG_TITLE + '\n\n' + locale.IG_ENTER_USERNAME);
        ...
        console.error('handleInstagramMenu error:', err);
...
async function handleInstagramUsername(ctx) {
    try {
        ...
        console.error('handleInstagramUsername error:', err);
        ctx.reply('An error occurred. Please try /start').catch(() => {});
